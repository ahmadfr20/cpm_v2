  ...
  <?php foreach ($shifts as $shift): ?>
      ...
      <table class="production-table table table-sm align-middle">
        ...
        <tbody class="shift-body" data-shift-id="<?= $shift['id'] ?>">
          <?php foreach ($shift['items'] as $item):
            $weightDc = (float)($shift['weight_dc_map'][(int)$item['product_id']] ?? 0);
            ...
            if ($machineActiveSlots !== null) {
                // Spesifik per mesin — gunakan custom minutes jika ada
                ...
                // Semua slot aktif (fallback pakai end_time_slot_id / total_minute)
                $tempPassed = false;
                foreach ($shift['slots'] as $s) {
                    if (!$tempPassed) {
                        $rowTotalMinute += (int)$s['minute'];
          ...
          ?>
          <tr class="item-row" data-row-total-minutes="<?= $rowTotalMinute ?>" data-original-target="<?= (int)$item['qty_p'] ?>" data-product-id="<?= (int)$item['product_id'] ?>" data-machine-id="<?= (int)$item['machine_id'] ?>" data-machine-code="<?= esc($item['machine_code'] ?? 'Mesin') ?>" data-part-name="<?= esc($item['part_name'] ?? '') ?>" data-weight-dc="<?= number_format($weightDc, 3, '.', '') ?>" data-weight-run="<?= number_format((float)($shift['weight_run_map'][$item['product_id']]??0), 3, '.', '') ?>" data-cavity="<?= (int)($shift['cavity_map'][$item['product_id']] ?? 1) ?>">
              <td class="sticky-left fw-bold text-center">
                ...
                <?php
                    // Kumpulkan operator per slot untuk mesin ini
                    $machineOpMap = []; // slot_id => operator_name
                    ...
                    // Tampilkan ringkasan operator unik
                    $uniqueOps = array_unique(array_values($machineOpMap));
                ?>
                <!-- Hidden inputs untuk operator per slot -->
              ...
              <?php 
                ...
                // Slot non-aktif (dari pilihan spesifik per mesin)
                if ($slotIsInactive) {
                    $targetSlot = 0;
                    ...
                    $targetSlot = $rowTotalMinute > 0
                      ? (int) round(((int)$item['qty_p'] / (float)$rowTotalMinute) * (float)$effectiveSlotMinute)
                      : 0;
                }
              ?>
              ...
              <?php if ($dandoriOnThisSlot !== null): ?>
                 <td class="slot-target-cell fw-bold bg-warning bg-opacity-25 text-center <?= $slotClass ?>" 
                    ...
                    <input type="number" class="form-control form-control-sm slot-input val-target text-center fw-bold text-primary p-0 bg-transparent border-0"
                           name="items[<?= esc($key) ?>][qty_target]" data-manual="<?= $isManual ?>"
                           value="<?= $displayTarget ?>" min="0">
                    <!-- Action button Akhiri Dandori (hidden by default, shown by JS) -->
              ...
              <?php endif; ?>
                ...
                <td class="text-start <?= $slotClass ?>">
                  <?php 
                    ...
                    if ($hasDandori) {
                        // Check if Dandori is already in dtDetail (prevent duplicate if saved with -1)
                        $found = false;
                  ...
                  <input type="hidden" name="items[<?= esc($key) ?>][downtime_penalty]" id="dtPenaltyInput_<?= esc($key) ?>" value="<?= (int)($exist['downtime_minute'] ?? 0) ?>" class="dt-penalty-val">
                </td>
              <?php endif; ?>
              <!-- END slot columns -->

              <?php 
                // Set flag pass end slot FOR NEXT iterations 
    ...
    <div class="mt-2 mb-5 p-3 border rounded bg-light summary-box" data-shift-id="<?= $shift['id'] ?>">
      ...
      <div class="mt-2 pt-2 border-top d-flex flex-wrap gap-3 align-items-center">
        ...
        <span class="text-muted small"><i class="bi bi-recycle"></i> Berat Scrap: <span class="total-weight-scrap fw-bold text-secondary">0.000</span> <span class="text-muted">KG</span></span>
      </div>
      <div class="mt-2 pt-2 border-top d-flex flex-wrap gap-3 align-items-center">
...
function recalculateTargets(shiftId) {
    ...
    shiftMinutesEl.innerText = `${totalShiftMinutes} Menit`;
    ...
    rows.forEach(row => {
        ...
        const rowTotalMinute = parseInt(row.dataset.rowTotalMinutes || totalShiftMinutes, 10);
        
        let sumTarget = 0;
        row.querySelectorAll('.slot-target-cell').forEach(cell => {
            // Abaikan slot non-aktif (yang opacity-50 atau td kosong yg di-generate)
            if (cell.innerText.trim() === '-' && cell.title === "Slot tidak aktif (per mesin)") return;

            const slotId = cell.dataset.slotId;
            const slotMinutes = parseInt(cell.dataset.slotMinutes || 0, 10);

            const targetInput = cell.querySelector('.val-target');
            if (targetInput) {
                const isManual = parseInt(targetInput.dataset.manual || 0, 10);
                if (!isManual) {
                    if (slotMinutes <= 0 || rowTotalMinute <= 0) {
                        targetInput.value = '0';
                    } else {
                        const newSlotTarget = Math.round(originalTarget * (slotMinutes / rowTotalMinute));
...
function recalcShiftSummary(shiftId) {
    ...
    let totalWeightFg = 0;
    let totalWeightNg = 0;
    let totalWeightScrap = 0;

    const machineSummaries = {}; // { machineId: { code: string, target: num, fg: num, ng: num, dt: num } }

    tbody.querySelectorAll('.item-row').forEach(row => {
        const weight = parseFloat(row.dataset.weightDc || 0) || 0;
        const weightRun = parseFloat(row.dataset.weightRun || 0) || 0;
        const cavity = parseInt(row.dataset.cavity || 1, 10) || 1;
        ...
        let rowScrapGram = (rowTarget / cavity) * weightRun;
        totalWeightScrap += rowScrapGram;
    ...
    if(box) {
        ...
        // Update Weight (Format to KG - dari gram / 1000)
        const weightFgSpan = box.querySelector('.total-weight-fg');
        const weightNgSpan = box.querySelector('.total-weight-ng');
        const weightScrapSpan = box.querySelector('.total-weight-scrap');
        
        const finalWeightFg = (totalWeightFg / 1000);
        const finalWeightNg = (totalWeightNg / 1000);
        const finalWeightScrap = (totalWeightScrap / 1000);
        ...
        if (weightScrapSpan) weightScrapSpan.innerText = finalWeightScrap.toLocaleString('id-ID', {minimumFractionDigits: 3, maximumFractionDigits: 3});
...
document.addEventListener('input', function(e) {
    if (e.target.classList.contains('val-fg') || e.target.classList.contains('val-target')) {
        ...
        if (e.target.classList.contains('val-target')) {
            e.target.dataset.manual = "1"; // Mark as manual so it doesn't get auto-overwritten
            // Re-sum target for row
            const row = e.target.closest('.item-row');
            let sumTarget = 0;
            row.querySelectorAll('.val-target').forEach(inp => sumTarget += parseInt(inp.value || 0, 10));
            const shiftTargetDisplay = row.querySelector('.target-shift-display');
            // if (shiftTargetDisplay) shiftTargetDisplay.innerText = sumTarget; // Target shift tetap terkunci
            
            // Add hidden input so backend can recalculate daily schedule qty_p easily
            ...
            if (!existingRowTotalTarget) {
                ...
                row.insertAdjacentHTML('beforeend', `<input type="hidden" name="row_targets[${dummyId}]" class="row-total-target-input" value="${sumTarget}">`);
            } else {
                existingRowTotalTarget.value = sumTarget;
            }
        }
        recalcShiftSummary(shiftId);
    }
...
function applySlotLock(){
  ...
  document.querySelectorAll('th.slot-header').forEach(th => {
     ...
     // Slot bisa diedit jika: bukan istirahat (atau admin override), dan waktu sesuai
     ...
     document.querySelectorAll(`.fg[data-shift-id="${shiftId}"][data-slot-id="${slotId}"]`).forEach(inp => {
         ...
         if(tdFg) {
             ...
             if (targetCell) {
                 const btnAkhiri = targetCell.querySelector('.btn-akhiri-dandori');
                 if (dandoriRunning && !isAdminOverride) {
                     ...
                     if (!countdownEl) {
                         ...
                         // Insert it before the button if button exists
                         if (btnAkhiri) targetCell.insertBefore(countdownEl, btnAkhiri);
                         else targetCell.appendChild(countdownEl);
                     }
     ...
     // Lock / Unlock button dan select didalam row slot DT
...
async function akhiriDandori(date, shiftId, machineId, timeSlotId) {
    ...
    // Save current active element in case we want to maintain focus, but reloading is simpler
    try {
        ...
        const res = await fetch('<?= site_url('die-casting/daily-production/endDandori') ?>', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (data.ok) {
            // Reload page is simplest to refresh slot properties
...
function openOperatorModal(shiftId, machineId, machineCode) {
    ...
    slots.forEach(slot => {
        ...
        // Cek apakah sudah ada nilai tersimpan
