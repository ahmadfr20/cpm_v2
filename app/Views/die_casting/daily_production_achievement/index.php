            ...
            <table class="table table-bordered table-sm align-middle">
                ...
                <?php foreach ($shift['items'] as $row): ?>
                    ...
                    <tr>
                        ...
                        $ngDetail = $shift['ng_detail_map'][$row['machine_id']][$row['product_id']] ?? [];
                        ?>
                        <td>
                            <input type="number"
                        ...
                        <td class="text-center align-middle">
                            ...
                            <div class="remark-hidden d-none" id="remarkHidden_<?= esc($key) ?>">
                                <?php 
                                $remarkMap = $shift['remark_map'][$row['machine_id']][$row['product_id']] ?? [];
                                foreach ($shift['slots'] as $slot): 
                                    $sId = (int)$slot['time_slot_id'];
                                    $rmk = $remarkMap[$sId] ?? '';
                                ?>
...
<script>
    ...
    function readNgHidden(key){
        ...
        Object.keys(map).sort((a,b)=>parseInt(a,10)-parseInt(b,10)).forEach(k=>{
            rows.push({ ng_category_id: parseInt(map[k].ng_category_id||'0',10), qty: parseInt(map[k].qty||'0',10) });
        });
        return rows;
    }
    ...
    function writeNgHiddenFromRows(key, rows){
        ...
        rows.forEach((r,i)=>{
            ...
            const a = document.createElement('input'); a.type='hidden'; a.name=`items[${key}][ng_details][${i}][ng_category_id]`; a.value=String(isNaN(ngId)?0:ngId); hidden.appendChild(a);
            const b = document.createElement('input'); b.type='hidden'; b.name=`items[${key}][ng_details][${i}][qty]`; b.value=String(isNaN(qty)?0:qty); hidden.appendChild(b);
        });
    }

    function calcTotalNg(rows){ return rows.reduce((s,r)=>{ const ngId=parseInt(r.ng_category_id||0,10),qty=parseInt(r.qty||0,10); return (ngId>0&&qty>0)?s+qty:s; },0); }
    function updateNgTotalUI(key,total){ const b=document.getElementById('ngTotalBadge_'+key); if(b) b.textContent=String(total); const n=document.getElementById('ngTotalInput_'+key); if(n) n.value=String(total); }
    ...
    function renderNgTable(key, shiftId, isEditable){
        ...
        if(rows.length === 0){
            ...
            rows.forEach((r, idx)=>{
                ...
                tr.innerHTML = `<td class="ng-no">${escapeHtml(cat?cat.ng_code:'-')}</td>
                    <td><select class="form-select form-select-sm ngSel editable-field" data-key="${escapeHtml(key)}" data-shift-id="${shiftId}" data-idx="${idx}" ${dis}>${buildNgSelectOptions(r.ng_category_id)}</select></td>
                    <td class="ng-qty"><input type="number" class="form-control form-control-sm ngQty editable-field" min="0" data-key="${escapeHtml(key)}" data-shift-id="${shiftId}" data-idx="${idx}" value="${parseInt(r.qty||0,10)}" ${dis}></td>
                    <td class="ng-act"><button type="button" class="btn btn-sm btn-danger py-0 px-2 ng-del-btn editable-field" onclick="deleteNgRowInline('${escapeHtml(key)}',${idx},${shiftId})" ${dis}><i class="bi bi-x"></i></button></td>`;
    ...
    function addNgRowInline(key, shiftId){ const rows=readNgHidden(key); rows.push({ng_category_id:0,qty:0}); writeNgHiddenFromRows(key,rows); renderNgTable(key,shiftId,true); }
    function deleteNgRowInline(key, idx, shiftId){ const rows=readNgHidden(key); rows.splice(idx,1); writeNgHiddenFromRows(key,rows); renderNgTable(key,shiftId,true); }

    document.addEventListener('change', function(e){
        const sel = e.target.closest('.ngSel');
        if(!sel) return;
        const key=sel.dataset.key, shiftId=sel.dataset.shiftId, idx=parseInt(sel.dataset.idx||'0',10);
        const rows=readNgHidden(key); if(!rows[idx]) return;
        rows[idx].ng_category_id=parseInt(sel.value||'0',10); writeNgHiddenFromRows(key,rows);
        const cat=NG_CATEGORIES.find(x=>x.id===rows[idx].ng_category_id)||null;
        const codeTd=sel.closest('tr')?.querySelector('.ng-no'); if(codeTd) codeTd.textContent=cat?cat.ng_code:'-';
        updateNgTotalUI(key,calcTotalNg(rows));
    });
    document.addEventListener('input', function(e){
        const inp=e.target.closest('.ngQty'); if(!inp) return;
        const key=inp.dataset.key, idx=parseInt(inp.dataset.idx||'0',10);
        let v=parseInt(inp.value||'0',10); if(isNaN(v)||v<0) v=0; inp.value=String(v);
        const rows=readNgHidden(key); if(!rows[idx]) return;
        rows[idx].qty=v; writeNgHiddenFromRows(key,rows); updateNgTotalUI(key,calcTotalNg(rows));
    });

    /* ===== DT INLINE ===== */
    function buildDtSelectOptions(selectedId){
        let html=`<option value="0">-- pilih --</option>`;
        DT_CATEGORIES.forEach(c=>{ const sel=(parseInt(selectedId||0,10)===c.id)?'selected':''; html+=`<option value="${c.id}" ${sel}>${escapeHtml(c.downtime_name)}</option>`; });
        return html;
    }
    function readDtHidden(key){
        const hidden=document.getElementById('dtHidden_'+key); const rows=[]; if(!hidden) return rows;
        const map={}; hidden.querySelectorAll('input').forEach(inp=>{ const m=inp.name.match(/\[dt_details\]\[(\d+)\]\[(downtime_category_id|downtime_minute)\]/); if(!m) return; const idx=m[1],field=m[2]; map[idx]=map[idx]||{}; map[idx][field]=inp.value; });
        Object.keys(map).sort((a,b)=>parseInt(a)-parseInt(b)).forEach(k=>rows.push({downtime_category_id:parseInt(map[k].downtime_category_id||'0',10),downtime_minute:parseInt(map[k].downtime_minute||'0',10)}));
        return rows;
    }
    function writeDtHiddenFromRows(key,rows){
        const hidden=document.getElementById('dtHidden_'+key); if(!hidden) return; hidden.innerHTML='';
        rows.forEach((r,i)=>{
    ...
    function renderDtTable(key, isEditable){
        const tbody=document.getElementById('dtBody_'+key); if(!tbody) return;
        const rows=readDtHidden(key); tbody.innerHTML='';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.ng-inline').forEach(el => {
            const key=el.dataset.key, btn=el.querySelector('.ng-add-btn');
            renderNgTable(key, 0, btn && !btn.disabled);
        });
        document.querySelectorAll('.dt-inline').forEach(el => {
            const key=el.dataset.key, btn=el.querySelector('.dt-add-btn');
    ...
    function openRemarkModal(key, shiftId) {
        ...
        if (slots.length === 0) {
            tbody.innerHTML = '<tr><td colspan="2" class="text-center text-muted py-2">Tidak ada slot waktu untuk shift ini.</td></tr>';
