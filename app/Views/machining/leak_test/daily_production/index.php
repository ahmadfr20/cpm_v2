...
.col-slot-remark{ width:360px; min-width:360px; } /* lebar untuk inline NG */
...
function calcTotals(){
  document.querySelectorAll('.production-table').forEach(t=>{
    ...
    const summary = t.closest('.table-scroll')?.nextElementSibling; // shift-summary
    if (summary) {
...
function parseSlotEnd(dateStr, start, end){
  ...
  // Tangani perpindahan hari (Shift Malam)
  const endHour = parseInt(String(end).split(':')[0], 10);
  if (endHour >= 0 && endHour < 7) {
      e.setDate(e.getDate() + 1);
  } else if (e <= s) {
...
function updateFinishShiftButton() {
  ...
  if (isAdmin) {
    btn.disabled = false;
    if (hint) hint.innerText = 'ADMIN: Finish Shift bisa dilakukan kapan saja (override waktu).';
    return;
  }

  // pakai slot terakhir shift terakhir
  ...
  if (hint) {
    hint.innerText = canFinish
      ? 'Finish Shift sudah aktif (slot terakhir sudah berakhir).'
      : `Finish Shift aktif setelah slot terakhir berakhir (${String(end).slice(0,5)}).`;
  }
}

updateFinishShiftButton();
setInterval(updateFinishShiftButton, 30000);
</script>
