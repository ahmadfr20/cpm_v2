...
.production-table{width:max-content;min-width:3200px;border-collapse:separate!important;border-spacing:0!important;table-layout:fixed}
.production-table th,.production-table td{font-size:13px;padding:8px 8px;white-space:nowrap;text-align:center;vertical-align:middle;box-sizing:border-box;line-height:1.2;background:#fff;transition: background 0.3s;}
...
.ng-mini-wrap{max-height:140px;overflow:auto;border:1px solid #e5e7eb;border-radius:10px;background:#fff}
...
.ng-mini-table th{position:sticky;top:0;z-index:2;background:#f8fafc;font-weight:900;text-align:center}
...
<form method="post" action="/machining/assy-shaft/hourly/store" id="assyShaftForm">
  <?= csrf_field() ?>
        ...
        <thead>
          <tr class="thead-row-1">
            ...
            <?php foreach ($shift['slots'] as $slot): ?>
              <th colspan="4" class="slot-header" data-start="<?= esc($slot['time_start']) ?>" data-end="<?= esc($slot['time_end']) ?>" data-shift-id="<?= $shift['id'] ?>" data-slot-id="<?= $slot['id'] ?>">
                ...
                <div class="form-check form-switch d-flex justify-content-center m-0 pb-1">
                  <input class="form-check-input rest-toggle border-secondary" type="checkbox" 
                         id="rest_as_<?= $shift['id'] ?>_<?= $slot['id'] ?>" 
                         data-shift-id="<?= $shift['id'] ?>" 
                         data-slot-id="<?= $slot['id'] ?>" 
                         data-slot-minutes="<?= $slot['minute'] ?>"
                         title="Tandai sebagai Jam Istirahat">
                  <label class="form-check-label ms-1 text-muted fw-normal" style="font-size: 11px; cursor:pointer;" for="rest_as_<?= $shift['id'] ?>_<?= $slot['id'] ?>">Rest</label>
                </div>
              </th>
            <?php endforeach ?>
          </tr>
              ...
              <td class="text-start outer-td">
                <div class="ng-inline" data-key="<?= esc($key) ?>">
                  <div class="ng-inline-head">
                    <div class="meta">Total NG: <span class="fw-bold text-danger" id="ngTotalBadge_<?= esc($key) ?>">0</span></div>
                    <button type="button"
                ...
                <input type="hidden" name="items[<?= esc($key) ?>][shift_id]" value="<?= (int)$sid ?>">
...
function applySlotLock(){
  ...
  document.querySelectorAll('th.slot-header').forEach(th => {
     ...
     const restToggle = document.getElementById(`rest_as_${shiftId}_${slotId}`);
...
document.querySelectorAll('.rest-toggle').forEach(t => {
   const storageKey = `rest_as_<?= esc($date) ?>_${t.dataset.shiftId}_${t.dataset.slotId}`;
...
function confirmFinishShift() {
  if (confirm('Yakin akan mengakhiri Shift Assy Shaft? Transaksi akan dikirim ke WIP Proses berikutnya dan jadwal shift ini tidak dapat diubah lagi.')) {
      const form = document.getElementById('assyShaftForm');
      form.action = '/machining/assy-shaft/hourly/finish-shift';
      form.submit();
  }
}
</script>
