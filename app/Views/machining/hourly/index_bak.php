          ...
          <tr class="item-row" data-original-target="<?= (int)$item['target_per_shift'] ?>">
            ...
            <?php foreach ($shift['slots'] as $slot):
              ...
              $targetSlot = $shift['total_minute'] > 0
                ? (int) round(((int)$item['target_per_shift'] / (float)$shift['total_minute']) * (float)$activeSlotMinute)
                : 0;
...
function recalculateTargets(shiftId) {
    ...
    const activeShiftMinutesForHeader = originalMinutes - restMinutes;
    shiftMinutesEl.innerText = `${activeShiftMinutesForHeader} Menit`;
    ...
    rows.forEach(row => {
        ...
        let machineDandoriTotal = 0;
        
        row.querySelectorAll('.slot-target-cell').forEach(cell => {
             machineDandoriTotal += parseInt(cell.dataset.dandoriMinutes || 0, 10);
        });

        const machineActiveMinutes = originalMinutes - restMinutes - machineDandoriTotal;
        
        let newShiftTarget = 0;
        if (machineActiveMinutes > 0) {
            newShiftTarget = Math.round(originalTarget * (machineActiveMinutes / originalMinutes));
        }
        ...
        toggles.forEach(t => {
            ...
            if (slotTargetCell) {
                const targetDisplay = slotTargetCell.querySelector('.slot-target-display');
                const slotDandoriMin = parseInt(slotTargetCell.dataset.dandoriMinutes || 0, 10);
                
                let activeSlotMin = slotMinutes - slotDandoriMin;
                if (t.checked) activeSlotMin = 0;

                if (activeSlotMin <= 0 || machineActiveMinutes <= 0) {
                    ...
                    const newSlotTarget = Math.round(newShiftTarget * (activeSlotMin / machineActiveMinutes));
  ...
  document.querySelectorAll('th.slot-header').forEach(th => {
     ...
     document.querySelectorAll(`.fg[data-shift-id="${shiftId}"][data-slot-id="${slotId}"]`).forEach(inp => {
         ...
         if(tdFg) {
             ...
             if (tdTarget && dandoriRunning && !isAdminOverride) {
                 ...
                 if (!countdownEl) {
                     ...
                     tdTarget.appendChild(countdownEl);
                 }
