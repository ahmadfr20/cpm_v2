...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 gap-2">
  <div>
      <h4 class="mb-0 text-dark fw-bold">DAILY SCHEDULE – LEAK TEST</h4>
      <small class="text-muted">Mengambil data dari proses WIP sebelumnya</small>
  </div>
  <div>
      <a href="<?= base_url('machining/leak-test/schedule/inventory?date='.$date) ?>" class="btn btn-primary fw-bold btn-sm rounded-pill px-3">
        <i class="bi bi-box-seam me-1"></i> Stock Area Leak Test
      </a>
  </div>
</div>
...
// PERBAIKAN JAVASCRIPT: Mengupdate text line ketika mesin berubah
$(document).on('change', '.machine-select', function() {
    const sel = this.closest('tr').querySelector('.product-select');
    const opt = this.selectedOptions[0]; // Tangkap pilihan yang dipilih
    const machineId = this.value;
    const lineTd = this.closest('tr').querySelector('.td-line'); // Tangkap kolom td-line
    const shiftId = sel.dataset.shift;
    
    if(!machineId) return;
    
    // Perbarui nomor line di tampilan
