...
class JigPlugHourlyController extends BaseController
{
    /* =========================
     * ADMIN CHECK
     * ========================= */
    ...
    /**
     * Get Jig Plug process ID (used as to_process_id in WIP)
     */
    private function getProcessIdJigPlug($db): int
    {
        $id = $this->getProcessIdByName($db, 'Jig Plug');
        if ($id <= 0) {
            // Fallback: cari dengan LIKE
            $row = $db->table('production_processes')->select('id')
                ->like('process_name', 'Jig Plug', 'both')->get()->getRowArray();
            $id = (int)($row['id'] ?? 0);
        }
        if ($id <= 0) throw new \Exception('Process "Jig Plug" belum ada di master production_processes');
        return $id;
    }

    /**
     * Get Machining process ID (used as from_process_id in WIP since Jig Plug is sub-process of Machining)
     */
    ...
    private function syncMachiningWipStockFromHourly($db, string $date): void
    {
        ...
        $jpProcessId    = $this->getProcessIdJigPlug($db);     // to_process_id = Jig Plug (7)
        $mcProcessId    = $this->getProcessIdMachining($db);   // from_process_id = Machining (2)
        ...
        // Build list of machine_id+product_id from Jig Plug schedule
        $scheduledPairs = [];
        foreach ($items as $si) {
        ...
        foreach ($items as $si) {
            ...
            // Jig Plug: from Machining → to Jig Plug
            $key = [
                $wipDateCol       => $date,
                'product_id'      => $productId,
                'from_process_id' => $mcProcessId,
                'to_process_id'   => $jpProcessId,
    ...
    private function finishMachiningTransferFlow($db, string $date, bool $forceAdmin = false): int
    {
        ...
        $wipDateCol   = $this->detectWipDateColumn($db);
        $jpProcessId  = $this->getProcessIdJigPlug($db);    // to_process_id = Jig Plug
        $mcProcessId  = $this->getProcessIdMachining($db);  // from_process_id = Machining
        ...
        // Build scheduled pairs for Jig Plug to filter actuals
        $scheduledPairs = [];
        foreach ($items as $si) {
            $scheduledPairs[] = (int)$si['machine_id'] . '_' . (int)$si['product_id'];
        }
        ...
        foreach ($items as $si) {
            ...
            // Transfer ke next process (resolve dari Jig Plug kalau ada, fallback ke Machining flow)
            $nextProcessId = $this->resolveNextProcessId($db, $productId, $jpProcessId);
            if (!$nextProcessId) {
            ...
            if ($nextProcessId > 0 && $transferQty > 0) {
                $this->upsertWipNextProcess(
                    $db, $date, $productId, $jpProcessId, (int)$nextProcessId,
    ...
    public function index()
    {
        ...
        $operators = $operatorModel->where('section', 'Jig Plug')->orderBy('operator_name', 'ASC')->findAll();
        ...
        return view('machining/jig_plug/hourly/index', [
            'date'         => $date,
            'operators'    => $operators,
            'shifts'       => $shifts,
            'canFinish'    => $canFinish,
