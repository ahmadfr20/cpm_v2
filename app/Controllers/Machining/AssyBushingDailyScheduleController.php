...
class AssyBushingDailyScheduleController extends BaseController
{
    ...
    public function store()
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Daily schedule Assy Bushing tersimpan. Non-admin hanya boleh mulai besok.');
        } catch (\Throwable $e) {
            $db->transRollback();
    ...
    /* =====================================================
     * incoming/assign WIP — ditambah guard juga (JSON)
     * ===================================================== */
    public function incomingWip()
    {
        // (tetap seperti kode kamu — tidak perlu guard, hanya read)
        $db   = db_connect();
    ...
    public function assignIncomingWip()
    {
        ...
        // sisanya biarkan sesuai kode kamu (yang sudah kamu tulis)
        // supaya tidak mengubah behavior existing.
        // -------------------------------------------------------
        // 👉 Kalau kamu mau, aku bisa kirim versi "assignIncomingWip"
        // full juga yang kamu paste sebelumnya + guard ini sudah nempel.
        // -------------------------------------------------------

        return $this->response->setJSON([
            'status' => false,
            'message' => 'Silakan gunakan fungsi assignIncomingWip kamu yang sudah ada; guard role+tanggal sudah ditambahkan di awal.'
        ]);
    }
    ...
    private function getProcessId($db, string $processName, ?string $processCode = null): ?int
    {
        if ($processCode && $db->fieldExists('process_code', 'production_processes')) {
            $row = $db->table('production_processes')->select('id')->where('process_code', $processCode)->get()->getRowArray();
        ...
        $row = $db->table('production_processes')->select('id')->like('process_name', $processName)->get()->getRowArray();
        if ($row) return (int)$row['id'];
