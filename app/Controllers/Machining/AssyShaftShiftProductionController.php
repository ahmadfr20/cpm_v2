...
class AssyShaftShiftProductionController extends BaseController
{
    public function index()
    {
        ...
        foreach ($shifts as $s) {
            ...
            $ngDetailMap = [];
            if ($db->tableExists('machining_assy_shaft_hourly_ng_details') && !empty($mappedItems)) {
                $hourlyRows = $db->table('machining_assy_shaft_hourly')
                ...
                if (!empty($hourlyIds)) {
                    $details = $db->table('machining_assy_shaft_hourly_ng_details d')
                        ->select('d.machining_assy_shaft_hourly_id, d.ng_category_id, d.qty')
                        ->whereIn('d.machining_assy_shaft_hourly_id', $hourlyIds)
                        ->get()->getResultArray();
                    ...
                    foreach ($details as $d) {
                        $hid = (int)$d['machining_assy_shaft_hourly_id'];
            ...
            $dtDetailMap = [];
            if ($db->tableExists('machining_assy_shaft_hourly_downtime_details') && !empty($mappedItems)) {
                $allHourlyRows = $db->table('machining_assy_shaft_hourly')
                ...
                if (!empty($allHourlyIds)) {
                    $dtRows = $db->table('machining_assy_shaft_hourly_downtime_details')
                        ->whereIn('machining_assy_shaft_hourly_id', $allHourlyIds)
                        ->get()->getResultArray();
        ...
        try {
            foreach ($items as $row) {
                ...
                if ($lastRow) {
                    ...
                    $db->table('machining_assy_shaft_hourly')->where('id', $lastRow['id'])->update([
                        'qty_fg' => $updatedFG,
                        'qty_ng' => $updatedNG,
                    ...
                    $db->table('machining_assy_shaft_hourly')->insert([
                        'production_date' => $date,
                        'shift_id' => $shiftId,
                        'time_slot_id' => $lastSlotId,
                        'machine_id' => $machineId,
                ...
                $allHourlyIds = $db->table('machining_assy_shaft_hourly')
                    ->select('id')
                    ->where([
                        'production_date' => $date,
                        'shift_id'        => $shiftId,
                ...
                if (!empty($allHourlyIds) && $db->tableExists('machining_assy_shaft_hourly_ng_details')) {
                ...
                if (is_array($remarks)) {
                    foreach ($remarks as $slotId => $rmk) {
                        ...
                        $hourlyRow = $db->table('machining_assy_shaft_hourly')
                            ->select('id')
                            ->where([
                                'production_date' => $date,
                                'shift_id'        => $shiftId,
    ...
    private function saveNgDetails($db, int $hourlyId, $ngDetails): void
    {
        ...
        if (!empty($batch)) {
            $db->table('machining_assy_shaft_hourly_ng_details')->insertBatch($batch);
        }
    }

    private function sumNgDetail($db, int $hourlyId): int
    {
        if (!$db->tableExists('machining_assy_shaft_hourly_ng_details')) return 0;
        $row = $db->table('machining_assy_shaft_hourly_ng_details')
                  ->select('SUM(qty) AS s')
                  ->where('machining_assy_shaft_hourly_id', $hourlyId)
                  ->get()->getRowArray();
        return (int)($row['s'] ?? 0);
